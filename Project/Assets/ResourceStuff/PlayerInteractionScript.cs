...
public class PlayerInteractionScript : MonoBehaviour
{
        ...
        if (CheckRange(CurrentPlayerPos, MouseWorldPos))
        {
            if (hits.Length > 0) // checks if object is present
            {
                foreach (var i in hits)
                {
                    ...
                    {
                        if (i.transform.GetComponent<TreeInteract>() || i.transform.GetComponent<StoneInteract>())//checks if it is a resource 
                        {
    ...
    private void PlaceWoodCollector()
    {
        ...
        Vector2 MouseWorldPos = Camera.main.ScreenToWorldPoint(MousePos);
        Vector2 gridPos;
        if (Init.Instance.resourceManager.GetWood() >= 100)//checks if there's enough  resources to build
        {
            gridPos = Init.Instance.grid.GetWorldCellPosition(MouseWorldPos.x, MouseWorldPos.y);
            var valid = Init.Instance.grid.BuildAtCell((int)gridPos.x, (int)gridPos.y, WoodCollector); // get grid postion of mouse

            if (valid)//checks if the building can be built
            {
    ...
    //private bool currenltyRepairing =false;
    private void repairTool()
    {
        ...
        if (hits.Length > 0) // checks if object is present
        {
            foreach (var i in hits)
            {
                ...
                {
                   // currenltyRepairing = true;
                  
                    Structure temp = i.transform.GetComponent<Structure>();
                    if (temp.repair()) 
                        {
                            Init.Instance.resourceManager.AddWood(-temp.woodRepairCost);
                            Init.Instance.resourceManager.AddStone(-temp.stoneRepairCost);
